<script>
    import { Modal, Button, Listgroup, ListgroupItem, Avatar, List, Li } from "flowbite-svelte";
    ...
    async function fetchComparingStats() {
        const { data } = await fetchGet(`${$BASE_URL}/api/users/${comparingWith.id}/statistics`, $tokenStore);
        comparingStats = data;
    }
    ...
    <div class="flex flex-col md:flex-row items-center justify-around">
                ...
                {#if userStats}
                    <List list="none">
                        <Li class="text-slate-950 font-light">
                            <span class={highlightClass(userStats.total_movies_watched, comparingStats?.total_movies_watched)}>Total movies watched: {userStats.total_movies_watched}</span>
                        </Li>
                        <Li class="text-slate-950 font-light">
                        ...
                        <Li class="text-slate-950 flex items-center font-light">
                            ...
                            <StarSolid class="ml-1 fill-current text-primary-700"/>
                        </Li>  
                        <Li class="text-slate-950 font-light">Most watched genre: {userStats.most_watched_genre}</Li>                  
                    </List>
                {/if}
            </div>
            <div>
                <h2 class="text-2xl font-bold mb-4">{comparingWith.username}</h2>
                {#if comparingStats}
                    <List list="none">
                        <Li class="text-slate-950 font-light">
                            <span class={highlightClass(comparingStats.total_movies_watched, userStats?.total_movies_watched)}>Total movies watched: {comparingStats.total_movies_watched}</span>
                        </Li>
                        <Li class="text-slate-950 font-light">
                        ...
                        <Li class="text-slate-950 font-light">Most watched genre: {comparingStats.most_watched_genre}</Li>
                    </List>
                {/if}
            </div>
        </div>
        ...
        {#if isOwner}
            <Listgroup class="w-48" active>
                <h3 class="p-1 text-center text-xl font-medium text-gray-900 dark:text-white">Following</h3>
