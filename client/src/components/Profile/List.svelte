...
{#each lists as list}
  <div class="flex justify-between bg-white shadow-md rounded-lg p-4 my-4 hover:bg-primary-100 cursor-pointer">
    <div class="content">
      ...
      <p class="text-sm">{list.description} - <b>{list.movie_count} movies</b></p>
    </div>
    {#if isOwner}
      <div class="actions flex flex-col items-end">
          <button class="bg-green-800 hover:bg-green-900 text-white rounded-md"><CirclePlusSolid /></button>
          <button class="bg-red-700 hover:bg-red-800 text-white rounded-md"><TrashBinSolid /></button>
      </div>
    {/if}
  </div>
{/each}
...
>
  <form on:submit={handleCreateList}>
    ...
    <button
      ...
      type="submit"
      on:click={() => (openCreateListModal = false)}
    >
      Create new list
    </button>
  </form>
