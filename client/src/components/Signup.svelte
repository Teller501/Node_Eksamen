<script>
    ...
    function validateForm() {
        ...
        if (!confirmDataProtectionPolicy) {
            toast.error("Please agree to the Data Protection Policy", {
                duration: 3000,
            });
            valid = false;
        }
    ...
    async function handleSignup(event) {
        ...
        if (status === 201) {
            ...
            const errorMessage =
                data && data.error ? data.error : "User creation failed.";
    ...
    async function checkUsernameAvailability() {
        ...
        const { data, status } = await fetchGet(
            `${$BASE_URL}/api/check-username/${username}`
        );
        if (status === 200) {
    ...
    <Card class="bg-slate-200">
        ...
        >
            ...
            <Label class="space-y-2">
                ...
                {#if password}
                    <span
                        class={`${strongPassword ? "text-green-700" : "text-red-700"}`}
                        >{strongPassword
                            ? "Strong Password"
                            : "Weak Password"}</span
                    >
            ...
            <Label class="space-y-2">
                ...
                {#if errors.confirmPassword}
                    <Helper class="text-red-700"
                        >{errors.confirmPassword}</Helper
                    >
                {/if}
            </Label>
            <Checkbox bind:checked={confirmDataProtectionPolicy}
                >I agree to the <A
                    class="ml-1"
                    on:click={() => (dataProtectionModal = true)}
