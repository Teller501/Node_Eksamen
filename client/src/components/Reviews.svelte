<script>
    import { Hr, Rating, Button, Avatar, A } from "flowbite-svelte";
    ...
    async function checkIfLiked(userId, reviewId) {
        const { data } = await fetchGet(
            `${$BASE_URL}/api/likes/${userId}/${reviewId}`,
            $tokenStore
        );
        return data;
    }
            ...
            reviews = reviews.map((review) => {
                if (review.id === reviewId) {
                    return {
                        ...review,
                        liked: true,
                        total_likes: Number(review.total_likes) + 1,
                    };
                }
                return review;
            });
                        ...
                        <div class="flex flex-col space-y-1">
                            ...
                            {#if review.user_id !== $userStore.id}
                                {#await checkIfLiked($userStore.id, review.id) then liked}
                                    {#if liked}
                                        <span
                                            class="text-gray-700 text-xs flex items-center"
                                        >
                                            <Button
