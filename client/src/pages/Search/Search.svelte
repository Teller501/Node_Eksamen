<script>
    ...
    import { Hr, A, Avatar, Modal, Spinner, Button } from "flowbite-svelte";
    ...
    <div class="w-full min-w-full p-4 bg-white shadow-md rounded-lg">
        {#if searchResults.length > 0}
            ...
            {#if type === "movies"}
                <div id="search-results" class="w-full flex flex-col space-y-4">
                    {#each searchResults as movie, index}
                        <div
                            class="flex flex-row items-center p-4 {index % 2 ===
                            1
                                ? 'bg-slate-200'
                                : 'bg-slate-50'} rounded-lg shadow-sm w-full"
                        >
                            <Movie
                            ...
                            />
                            <h3
                                class="text-slate-900 ml-4 text-lg font-medium flex-grow"
                            >
                                <A
                                    ...
                                    >{movie.title}
                                    <span
                                        class="text-gray-500 ml-2 font-light text-sm"
                                        >{`${movie.original_title !== movie.title ? `(${movie.original_title})` : ""}`}</span
                                    ></A
                                >
                            </h3>
                        </div>
                        {#if index < searchResults.length - 1}
                            <Hr
                                hrClass="my-4 border-t-0 border-slate-300 w-full"
                            />
                        {/if}
                    {/each}
                </div>
            {:else if type === "users"}
                <div
                    id="search-results"
                    class="w-full min-w-full flex flex-col space-y-4"
                >
            ...
            {:else}
                ...
                >
                    {#each searchResults as list, index}
                        ...
                        >
                            <h3
                                class="text-primary-800 ml-4 text-lg font-medium flex-grow"
                            >
                                <Button
                                    type="button"
                                    on:click={() => fetchListMovies(list)}
                                    class="bg-transparent hover:bg-transparent active:ring-0 focus:ring-0 text-lg text-primary-700 hover:text-blue-500 
                                    {index % 2 === 1
                                        ? 'bg-slate-200'
                                        : 'bg-slate-50'}"
                                >
